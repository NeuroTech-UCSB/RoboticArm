...
const float SERVO_MIN_ANGLE[NUM_SERVOS] = {0, 45, 90, 50};
...
void loop() {
  ...
  if ( cmd_state == PUSH ) {
    if (SERVO_ANGLES[SERVO2] > SERVO_MIN_ANGLE[SERVO2] && SERVO_ANGLES[SERVO3] > SERVO_MIN_ANGLE[SERVO3] && Pushing) {
      writeAngle(SERVO2, SERVO_ANGLES[SERVO2] - 0.1); 
      writeAngle(SERVO3, SERVO_ANGLES[SERVO3] - 0.1); //direction is reversed for servo 3 
      //Serial.println("Pushing");
      if (SERVO_ANGLES[SERVO2] < 60) {
        if (SERVO_ANGLES[CLAW_SERVO] <= SERVO_MAX_ANGLE[CLAW_SERVO]) {
          writeAngle(CLAW_SERVO, SERVO_ANGLES[CLAW_SERVO]-0.1);
        }
      }
      if (SERVO_ANGLES[CLAW_SERVO] >= 60) {
        if (SERVO_ANGLES[CLAW_SERVO] <= SERVO_MAX_ANGLE[CLAW_SERVO]) {
          writeAngle(CLAW_SERVO, SERVO_ANGLES[CLAW_SERVO]+0.1);
        }
      }
      ...
      if (SERVO_ANGLES[SERVO2] <= SERVO_MAX_ANGLE[SERVO2] && SERVO_ANGLES[SERVO3] <= SERVO_MAX_ANGLE[SERVO3] && Pulling) {
        writeAngle(SERVO2, SERVO_ANGLES[SERVO2] + 0.1); 
        writeAngle(SERVO3, SERVO_ANGLES[SERVO3] + 0.1); //direction is reversed for servo 3 
  ...
  else if ( cmd_state == LEFT && Rotating) {
...
/*
   ...
   else if (cmd_state == CLOSE && Closing) {
